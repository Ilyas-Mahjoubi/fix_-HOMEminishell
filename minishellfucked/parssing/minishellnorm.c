...
/* t_token *tokenize_input(char *input)
{
	...
	while (input[i])
	{
		...
		{
			current_word_token = NULL; // Reset current word since we're starting a new token
			//head = handle_env_variable(input, &i, head, get_last_token(head));
			head = handle_env_variable(input, &i, head, &current_word_token);
...
/* t_token *tokenize_input(char *input, t_info *info)
{
    ...
    return (head);  // Return the list of tokens
} */ // back here this for segmantatio fault when the double quotes is open"
